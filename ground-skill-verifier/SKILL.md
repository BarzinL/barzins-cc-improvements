...
Operationalizes the pre-implementation grounding pass in `CLAUDE.md` and
`docs/research/coding/pre-implementation-grounding.md`. That protocol is correct but
aspirational — it relies on the operator re-prompting "ground further" 2–4× per step. This
skill makes it **self-terminating** (a ledger with a termination test), **self-resolving**
(a decision policy for forks), and **independently verified** (a cold second agent), so it
runs to completion without re-prompting.

## Core invariant

Do not return to the user while any load-bearing claim is UNVERIFIED or any
code-determinable fork is unresolved. "Looks done" is not a termination criterion — an empty
UNVERIFIED column is.
...
## 2. The five moves (from CLAUDE.md)

1. Read breadth-first — topology: imports, calls, where state lives (graphify query for fan-out
   questions, else grep/Read).
...
5. Write findings into the plan doc.
...
all load-bearing claims. This is the "2–4 passes" — now self-driven.
...
- **Code-determinable** — one branch is correct given how the code wires, a project invariant, or
  ...
  alternative(s)* into the plan doc's decision log. Do not ask.
...
- **(b) Gap-scan** — independently hunt load-bearing claims the ledger omits: bypass paths,
  untested assumptions, unnamed exclusions, count claims that are off. For high-stakes steps it
  builds its own ledger and diffs against yours.
...
comes from cold context + adversarial stance, not model diversity (same model is fine).
...
2. **What I did** — plain English, same altitude. ("The turn loop now gets its handle from
   `for_user(owner)`…", not "modified `_run_turn:412`".)
3. **Decisions + why + rejected alternatives** — one line each, plain English.
4. **Genuine forks** for the user, if any.
5. **One-command behavioral proof** — before/after, a single command the user can run. Every
   load-bearing behavioral claim in the prose cites a runnable check, so the summary is anchored
   to ground truth, not floating prose.
...
everything and stop before commit** for sign-off. Ceiling 1: also commit autonomously (move here
once the decision log shows self-resolved forks landing where the user would). Ceiling 3: widen
the preference class, escalate more.
...
- Reactively patching when a check fails — read the code and reason first (CLAUDE.md).
